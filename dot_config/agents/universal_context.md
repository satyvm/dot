...
- **Toolchain & Container Management**:
  - **`mise`**: Used locally per directory/repository (via `mise.toml`, `.mise.toml`, or `.tool-versions`) to manage and isolate tool and runtime versions, backed by global shims (`~/.local/share/mise/shims`).
  - **Docker**: Container runtime on macOS is managed and started via **Colima** (`colima start`, `docker context use colima`).
